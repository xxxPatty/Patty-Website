...
function getBotResponse(text) {
    botResponse = `<img class="botAvatar" src="./static/img/patty_avatar.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}
...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) {
            ...
            const fallbackMsg = "出了點問題，等等再試。";

            const BotResponse = `<img class="botAvatar" src="./static/img/patty_avatar.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
...
function actionTrigger() {
    $.ajax({
        url: head_url+`:5005/conversations/${sender_id}/execute`,
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({
            name: action_name,
